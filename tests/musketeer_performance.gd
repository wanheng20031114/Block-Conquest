...
func measure(kind: String) -> void:
	...
	while Time.get_ticks_usec()-start<5000000:
		...
		previous=now
	samples.sort()
	results[kind]={"units":500,"shooters":250,"median_ms":samples[samples.size()/2],"p95_ms":samples[int(samples.size()*.95)],"frames":samples.size(),"shots":pool.launch_count-shots,"peak_flights":pool.peak_active,"visuals":pool.get_child_count(),"effect_peak":game.get_node("EffectPool").peak_active,"effect_dropped":game.get_node("EffectPool").dropped,"effect_coalesced":game.get_node("EffectPool").coalesced}
	print("MUSKETEER_PERFORMANCE ",kind," ",JSON.stringify(results[kind]))
func _run() -> void:
	...
	FileAccess.open("res://.local/musketeer-20260914/performance-results.json",FileAccess.WRITE).store_string(JSON.stringify(results,"\t"))
