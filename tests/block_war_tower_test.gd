...
func _audit_upgrade() -> void:
	...
	_check(tower.level == 2 and tower.population == 70.0 and game.tower_range(tower) == 12.0, "A paid level-two upgrade immediately changes the tower's combat level")
	_spawn(Vector3(4, 0, 0), 1, 6)
	game.marches.tick(0.25)
	game.simulate(0.001)
	...
	game.upgrade_selected()
	_check(tower.level == 3 and tower.population == 10.0 and game.tower_range(tower) == 13.0, "A paid level-three upgrade keeps the existing costs and extends range")
	game.simulate(1.201)
