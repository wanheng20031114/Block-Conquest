...
func _run() -> void:
	...
	var dispatch_target: WarBuilding = game.buildings[2]
	home().population = 60.0
	clear_sounds()
	mouse(screen(home()), true)
	check(sounds.is_empty(), "pressing a dispatch source stays silent until click or order release")
	for building: WarBuilding in [dispatch_target, home(), dispatch_target]:
		motion(screen(building))
		game._update_drag(screen(building))
	game.set_percentage(50)
	check(sounds.is_empty(), "dragging the line across targets and changing its amount is silent")
	mouse(screen(dispatch_target), false)
	check(heard(&"war_order") == 1 and sounds.size() == 1, "valid dispatch release produces exactly one block tap")
	game.marches.clear()
	clear_sounds()
	mouse(screen(home()), true)
	mouse(screen(home()), false)
	check(heard(&"war_select") == 1 and sounds.size() == 1, "a plain building click produces exactly one light ta")
	clear_sounds()
	mouse(screen(home()), true)
	motion(screen(dispatch_target))
	game._update_drag(screen(dispatch_target))
	...
	game.marches.tick(.3)
	check(game.cast_ground_skill(2, game.marches._units[0].position), "ground recall has eligible troops")
	...
	check(game.cast_skill(3, plan.source), "rabbit enchants its own source")
	var activation: Dictionary = sounds.filter(func(item: Dictionary): return item.kind == &"war_rabbit_burrow")[0]
	check(heard(&"war_rabbit_burrow") == 1 and activation.position.distance_to(plan.source.global_position) < .001 and activation.spatial, "R activation sounds at its enchanted source")
	clear_sounds()
	check(game.issue_order(plan.source, plan.target, 100) == 50, "ordinary order starts the prepared tunnel")
	check(heard(&"war_rabbit_burrow") == 0, "digging does not announce an unopened exit")
	game.audio.play_world(&"war_march", plan.exit)
	game.simulate(plan.dig_duration + .001)
	...
	check(feedback.get_node("select").stream.get_stream(0).resource_path == "res://assets/audio/ui/arc_nice_click.wav", "menus use the unchanged arc-nice click asset")
	near(feedback.get_node("select").volume_db, -8.0, "menu click retains arc-nice's restrained player gain")
