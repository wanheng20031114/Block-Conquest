...
func _run() -> void:
	...
	game.simulate(2.0)
	near(game.by_id[0].population, 68.0, "ally support belongs to the receiving residence")
	...
	near(game.by_id[2].population, 64.0, "conversion retains recruitment only until completion")
	await reset_match()
	for faction: int in 6:
	...
	game.simulate(7.99)
	check(game.shields.size() == 6, "all walls remain active just before eight seconds")
	game.simulate(0.01)
	...
	game.cast_ground_skill(1, Vector3(10, 0, 0), 3)
	check(game.drag_source == game.by_id[0], "AI haste preserves a troop dispatch drag")
	check(game.marches.haste_zones.has(3) and not game.marches.haste_zones.has(0), "haste field belongs only to its caster")
	game._cancel_drag()
	...
	check(game.marches.haste_zones[1].at.distance_to(Vector3(-17.3, 0, 0)) < 0.3, "AI places W ahead of the actual marching group")
	game.marches.clear()
	expose(1, 12, Vector3(-20, 0, -12), Vector3(20, 0, -12), 0)
	expose(1, 12, Vector3(-20, 0, 12), Vector3(20, 0, 12), 0)
	check(TACTICS.new(1)._haste_target(game, visible_units()).is_empty(), "distant small armies cannot combine into a global W score")
	await reset_match()
	game.elapsed = 10.0
	...
	ai.take_turn(game)
	check(game.faction_skills[1].cooldowns[3] == 70.0, "computer casts paid fire at a dense unescorted formation")
	await reset_match()
	expose(0, 24, Vector3(-3, 0, 2), Vector3(28, 0, 2), 0)
