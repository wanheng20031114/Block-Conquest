extends SceneTree
## Two authored heavy guns round-trip through the actual protocol and replica path.
...
func _run() -> void:
	...
	var fortress: BattleBuilding = host.spawn_building("heavy_fortress",0,Vector3.ZERO)
	var hidden: BattleBuilding = host.spawn_building("heavy_fortress",2,Vector3(40,0,40))
	var ally: BattleBuilding = host.spawn_building("heavy_fortress",1,Vector3(12,0,0),true)
	var target: BattleUnit = host.spawn_unit("war_elephant",2,Vector3(0,0,-12))
	host.visible_ids.append(target.entity_id)
	host.elapsed = 2
	for i: int in 2:
		fortress.weapons[i].last_fired = [1.2,1.5,1.8][i]
		fortress.artillery.guns[i].turret.rotation.y = [-.7,.6,1.1][i]
		fortress.artillery.guns[i].elevation.rotation.x = [-.12,.0,.2][i]
	ally.construction_progress = .5
	for i: int in 2: target.receive_hit(DamageResolver.snapshot(fortress._stats,0,0,0),fortress)
	var snapshot := wire(sender.build_snapshot(0))
	var state := state_for(snapshot,fortress.entity_id)
	check(NetworkProtocol.VERSION == 18,"battery array contract has explicit protocol version")
	check(state.turrets.size() == 2,"exactly two public gun states")
	for i: int in 2:
		check(state.turrets[i].size() == 3 and state.turrets[i].fired == fortress.weapons[i].last_fired,"gun release time survives wire without target or cooldown: %d" % i)
	check(state_for(snapshot,hidden.entity_id).is_empty(),"hidden enemy fortress omitted")
	check(not state_for(snapshot,ally.entity_id).has("production"),"allied fortress omits private construction data")
	...
	check(receiver.last_received_tick == 0,"client accepts real fortress through snapshot decoder")
	var replica: BattleBuilding = client.entities_by_id[fortress.entity_id]
	var client_target: BattleUnit = client.entities_by_id[target.entity_id]
	receiver.render(.1)
	for i: int in 2:
		var gun := replica.artillery.guns[i]
		check(is_equal_approx(gun.turret.rotation.y,fortress.artillery.guns[i].turret.rotation.y) and is_equal_approx(gun.elevation.rotation.x,fortress.artillery.guns[i].elevation.rotation.x),"replica gun independently aims: %d" % i)
		check(is_equal_approx(gun.animation.current_animation_position,receiver._playback_time-fortress.weapons[i].last_fired),"replica independently samples release phase on interpolated clock: %d" % i)
	...
	check(client_target.hp == 206,"two client projectiles cannot duplicate authority damage")
	...
	for invalid: Variant in invalid_arrays:
		...
		bad.tick = 1
		state_for(bad,fortress.entity_id).turrets = invalid
	...
	for invalid: Variant in [null,{},{"yaw":NAN,"pitch":0,"fired":-1},{"yaw":0,"pitch":1,"fired":-1},{"yaw":0,"pitch":0,"fired":2.1},{"yaw":0,"pitch":0,"fired":-.5},{"yaw":0,"pitch":0,"fired":0,"target":1}]:
		var bad := snapshot.duplicate(true)
		state_for(bad,fortress.entity_id).turrets[1] = invalid
	...
	state_for(bad_kind,fortress.entity_id).kind = "cannon_tower"
	check(not receiver._valid_snapshot(bad_kind),"single cannon tower cannot accept two gun states")
	...
	fortress.weapons[0].last_fired = 2.9
	fortress.artillery.guns[0].turret.rotation.y = -.9
	...
	check(is_equal_approx(replica.artillery.guns[1].animation.current_animation_position,1.5),"other guns return to rest independently")
	check(client_target.hp == 206,"later snapshots preserve authority HP")
	client.queue_free(); host.queue_free()
	await process_frame
	await process_frame
	DirAccess.make_dir_recursive_absolute("res://.local/defenses/heavy_fortress")
	FileAccess.open("res://.local/defenses/heavy_fortress/network.json",FileAccess.WRITE).store_string(JSON.stringify({"checks":checks,"failures":failures},"\t"))
	print("FORTRESS_NETWORK ",checks," checks; ",failures.size()," failures")
