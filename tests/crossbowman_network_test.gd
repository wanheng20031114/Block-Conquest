...
func _run() -> void:
	...
	var shooter: BattleUnit = host.spawn_unit("crossbowman",0,Vector3.ZERO)
	var target: BattleUnit = host.spawn_unit("shield_guard",2,Vector3(0,0,-5))
	var hidden: BattleUnit = host.spawn_unit("crossbowman",2,Vector3(30,0,30))
	host.visible_ids.assign([target.entity_id])
	var barracks: BattleBuilding = host.spawn_building("barracks",0,Vector3(10,0,10))
	check(barracks.production.recruit("crossbowman").ok,"crossbow queued in host barracks")
	shooter._model.strike()
	shooter._model.prepare_attack_release(.2)
	var flight := ProjectileFlight.new()
	flight.initialize(host,shooter,target,DamageResolver.snapshot(shooter._stats,0,0,0),"bolt")
	check(flight._start.distance_to(shooter.get_projectile_origin())<.0001,"host samples authored muzzle")
	flight.advance(1)
	check(target.hp == 140 and host.effects == 1,"host applies one penetrative bolt")
	...
	check(receiver.last_received_tick == 0,"client accepts crossbow state")
	...
	check(not replica.is_physics_processing() and target_replica.hp == 140,"replica cannot autonomously attack")
	var cosmetic := ProjectileFlight.new()
	cosmetic.initialize_visual(client,flight._start,flight._end,"bolt",.2,.13,target_replica)
	cosmetic.advance(1)
	check(target_replica.hp == 140,"client bolt presentation never deals damage")
	check(state_for(snapshot,barracks.entity_id).production.training[0].kind == "crossbowman","recruit queue transmitted")
	for invalid: Dictionary in [{"kind":"missing_unit"},{"anim":"repair"},{"anim":"heal"},{"anim":"build"},{"anim":"gather"},{"phase":NAN},{"attack_range":8}]:
		...
		check(receiver.last_received_tick == 0,"reject mismatched crossbow state "+str(invalid))
	# An already launched bolt retains its penetration after the shooter is gone.
	var after_death := ProjectileFlight.new()
	after_death.initialize(host,shooter,target,DamageResolver.snapshot(shooter._stats,0,0,0),"bolt")
	shooter.queue_free()
	await process_frame
	after_death.advance(1)
	check(target.hp == 135,"source deletion leaves launch snapshot valid")
	client.queue_free()
	host.queue_free()
	await process_frame
	await process_frame
	DirAccess.make_dir_recursive_absolute("res://.local/crossbow-20260914")
	FileAccess.open("res://.local/crossbow-20260914/network-results.json",FileAccess.WRITE).store_string(JSON.stringify({"checks":checks,"failures":failures},"\t"))
	print("CROSSBOW_NETWORK ",checks," checks; ",failures.size()," failures")
