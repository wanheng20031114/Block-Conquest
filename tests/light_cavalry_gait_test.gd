...
func _run() -> void:
	...
	if models.is_empty(): models = PackedStringArray(["light_cavalry"])
	for model_kind: String in models:
		assert(model_kind in ["light_cavalry", "light_cavalry_advanced"])
		for folder: String in ["","batched/"]:
			var model: UnitVisual=load("res://assets/models/units/"+folder+model_kind+".tscn").instantiate()
