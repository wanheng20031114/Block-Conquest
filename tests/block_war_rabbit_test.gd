...
func _run() -> void:
	...
	game.marches.send(0, 1, 0, 1, PackedVector3Array([center, center + Vector3(80, 0, 0)]))
	...
	game.marches.tick(5.8)
	check(rushing.rush_remaining == 0.0 and game.marches.haste_zones.is_empty(), "squad rush expires at six seconds without a ground haste zone")
