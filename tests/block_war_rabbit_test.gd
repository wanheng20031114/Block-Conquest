...
func _run() -> void:
	...
	if not plan.is_empty():
		...
		check(game.marches.get_units().is_empty(), "warning conceals passengers")
		game.simulate(1.19)
		check(game.marches.get_units().is_empty(), "full warning respected")
		game.simulate(0.02)
		check(game.marches.get_units().size() == 6, "first batch has six soldiers")
		game.set_paused(true)
