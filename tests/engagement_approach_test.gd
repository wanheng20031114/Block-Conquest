...
const KINDS: Array[String] = ["swordsman", "shield_guard", "spearman", "farmer", "knight", "war_elephant", "archer", "crossbowman", "musketeer"]
	...
	for rate: int in [30, 60]:
		...
		for direct: bool in [true, false]:
			...
			if direct:
				...
				for kind: String in ["swordsman", "spearman", "shield_guard", "light_cavalry", "crossbowman", "musketeer"]:
