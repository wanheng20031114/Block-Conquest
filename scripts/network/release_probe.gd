...
func validate_resource_values() -> void:
	...
	check(priest.cost==180 and priest.hp==70 and priest.damage==3 and priest.melee_armor==0 and priest.ranged_armor==1
		and priest.speed==3.5 and priest.sight==14 and priest.supply==2 and priest.military and priest.cooldown==1.5
		and priest.attack_windup_seconds==.25 and priest.range==.8 and priest.radius==.45
		and priest.combat_class==&"infantry" and priest.projectile.is_empty() and priest.bonuses.is_empty(), "packaged_priest_combat_values")
	check(priest.support_kind==&"heal" and priest.support_range==6 and priest.support_amount==10 and priest.support_period==1
		and priest.support_windup_seconds==.6 and not priest.support_auto_chase, "packaged_priest_healing_contract")
	...
	check(elephant.cost==300 and elephant.hp==360 and elephant.damage==32 and elephant.melee_armor==2
		...
		"packaged_war_elephant_approved_values")
	...
	check(heavy.hp == 260 and heavy.cost == 500 and heavy.damage == 100 and heavy.bonuses == {&"building": 100}
		...
		and heavy.range == 14 and heavy.min_range == 3.5 and heavy.cooldown == 4.2
		and heavy.attack_windup_seconds == .45 and heavy.projectile == "cannon" and heavy.combat_class == &"siege"
		and heavy.splash_radius == 0 and heavy.military and heavy.training_seconds == 36,
		"packaged_heavy_cannon_approved_values")
	check(heavy.cannon_range_upgrades and cannon.cannon_range_upgrades and not catapult.cannon_range_upgrades,
		"packaged_barrel_research_explicit_eligibility")
	var heavy_shot := DamageResolver.snapshot(heavy, 0, 0, 0)
	check(DamageResolver.resolve(heavy_shot, BalanceCatalog.building("barracks")) == 190
		and DamageResolver.resolve(heavy_shot, elephant) == 97, "packaged_heavy_cannon_single_target_damage")
