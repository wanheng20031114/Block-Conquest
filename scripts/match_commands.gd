...
func execute(command: Dictionary, owner: int) -> Dictionary:
	...
	if not ids is Array or ids.size() > game.command_unit_limit(owner):
	...
	match kind:
		...
		"build":
			...
			if building_type not in ["headquarters", "barracks", "factory", "academy", "defense_tower", "cannon_tower", "castle", "heavy_fortress"]:
				return failure("无效建筑")
