...
const BUILD_ORDER := ["barracks", "factory", "academy", "defense_tower", "cannon_tower", "castle", "heavy_fortress", "headquarters"]
...
func _ready() -> void:
	...
	portraits["cannon_range_upgrade"] = portraits["cannon"]
	portraits["recovery_upgrade"] = portraits["farmer"]
...
func refresh_hotkey_labels() -> void:
	...
	%BuildButton.text = "建造箭塔  [" + game.settings.hotkey_text("rts_build_tower") + "]"
	...
	$HelpOverlay/Paper/Economy.text = "兵营训练步兵与骑兵，军工厂生产攻城器和工程兵，学院训练牧师并研究科技。详细数值见战争图鉴。\n每矿%d位 · 每人%s秒+%d金 · 攻击科技%s，防御科技%s，攻城近甲固定0 · 训练和研究队列可取消退款" % [ResourceVein.CAPACITY, BalanceCatalog.ECONOMY.mining_seconds, BalanceCatalog.ECONOMY.mining_gold, _upgrade_bonus_text("attack"), _upgrade_bonus_text("defense")]
...
func refresh() -> void:
	...
	if game.selection.is_empty():
		...
		if entity.is_in_group("units"):
			...
			selected_role.text += " · " + definition.formation_label() + " · " + definition.role_label()
			...
			if definition.armor_penetration > 0.0:
				selected_stats.text += " · 穿甲%d" % definition.armor_penetration
			...
			if entity.owner_id == game.local_owner_id:
				...
				if not entity.is_constructed:
					selected_stats.text = "施工 %d%% · 农民右键接手\n已付 %d 金 · 取消返还 %d 金" % [roundi(entity.construction_progress * 100), entity.actual_paid_gold, entity.construction_refund()]
...
func _refresh_actions() -> void:
	...
	if workers:
		...
		if game.build_mode:
			%RecruitHint.text = "下一座%s · %d 金 · Shift 连续建造" % [BalanceCatalog.building(game.build_kind).name, game.get_player(game.local_owner_id).get_building_cost(game.build_kind)]
		for kind: String in BUILD_ORDER:
			var definition := BalanceCatalog.building(kind)
			var player: PlayerState = game.get_player(game.local_owner_id)
			var hint: String = "%d 秒施工" % definition.build_seconds
			if kind == "defense_tower":
				hint += " · 第 %d 座付费建造 · 取消或摧毁不重置造价" % (player.paid_tower_count + 1)
			_actions.append({"kind": "build", "id": kind, "portrait": kind, "name": definition.name, "cost": player.get_building_cost(kind), "hint": hint})
	elif building != null:
		...
		elif building.building_type in ["defense_tower", "cannon_tower", "castle", "heavy_fortress"]:
		...
		%RecruitHint.text = "右键设置集结点 · 研究取消全额退款" if building.building_type == "academy" else ("自动攻击射程内敌人 · 无法驻军" if building.building_type in ["defense_tower", "cannon_tower", "castle", "heavy_fortress"] else "右键设置集结点")
		if building.get_combat_definition().splash_radius > 0.0:
			%RecruitHint.text = "自动炮击 · 爆炸半径 %.1f · 无友伤" % building.get_combat_definition().splash_radius
...
func _set_preview_hover(kind: String) -> void:
	if kind == "army_capacity_upgrade":
		...
		kind = "gold_vein"
	elif kind == "cannon_range_upgrade":
		kind = "cannon"
	elif kind == "recovery_upgrade":
		kind = "farmer"
