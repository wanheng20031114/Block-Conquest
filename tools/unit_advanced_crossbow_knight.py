...
def fitted_front_panel(skin, rows, columns=7, inset=.025):
    """Project a closed plate onto the actual faceted horse, offline only.

    Rows are (height, half-width). Intersect projected triangles directly so
    ...
    for y, half_width in rows:
        for x in np.linspace(-half_width, half_width, columns):
            q = np.array((x, y)) - a[:, :2]
            b = (q[:, 0]*v[:, 1]-q[:, 1]*v[:, 0])/determinant
            c = (u[:, 0]*q[:, 1]-u[:, 1]*q[:, 0])/determinant
            inside = (b >= -1e-7) & (c >= -1e-7) & (b+c <= 1+1e-7)
            assert inside.any(), f'Armor point outside horse surface: {x}, {y}'
