...
s.node('Resources','Panel','.',**rect(-374,24,350,76,anchor_right=True))
label('GoldCaption','Resources','◈  金币',18,8,72,22,12,'Color(0.87,0.74,0.45,1)')
label('GoldValue','Resources','320',18,32,154,36,28,'Color(1,0.86,0.53,1)',True,True,clip_text='true')
label('Income','Resources','自然 +1/秒',95,9,80,21,11,'Color(0.69,0.66,0.51,1)')
label('ArmyCaption','Resources','人口 · 含训练',194,8,140,18,11,'Color(0.67,0.77,0.8,1)')
label('ArmyValue','Resources','军事 0 / 60',194,29,140,20,15,unique=True)
label('FarmersValue','Resources','农民 3 / 10',194,50,140,20,15,unique=True,mouse_filter='1')
...
label('MapTitle','MapFrame','琥珀十字路',12,6,170,24,12,'Color(0.8,0.77,0.63,1)')
...
for index, (name, hotkey, cost) in enumerate(zip(['剑士','弓箭手','骑士','投石车','加农炮','农民'],['Q','E','R','T','Y','U'],[45,60,80,140,180,50])):
    ...
    label('Name',path,name,4,83,78,23,13,bold=True,horizontal_alignment='1')
...
label('Toast','.','发展领地，率军出征',-360,164,720,38,17,'Color(1.0,0.91,0.68,1)',unique=True,anchor_left='0.5',anchor_right='0.5',horizontal_alignment='1')
...
label('ResultHeading','ResultOverlay/Paper','战场属于你',32,94,616,53,35,bold=True,unique=True,horizontal_alignment='1')
...
m.ext('PackedScene','scenes/resource_vein.tscn','mine')
m.ext('PackedScene','scenes/building_preview.tscn','ghost')
m.ext('Script','scripts/construction_navigation.gd','construction_nav')
...
m.node('积木争霸','Node3D',None,script='ExtResource("game")')
m.node('WorldEnvironment','WorldEnvironment','.',environment='SubResource("world")')
m.node('Sun','DirectionalLight3D','.',rotation_degrees='Vector3(-54,-31,0)',light_color='Color(1,0.89,0.72,1)',light_energy='1.15',shadow_enabled='true',directional_shadow_max_distance='135.0',shadow_bias='0.06',shadow_normal_bias='2.0',directional_shadow_blend_splits='true',directional_shadow_mode='0',light_angular_distance='1.6')
m.node('CameraRig','Node3D','.',position='Vector3(-16,0,21)',script='ExtResource("camera")',physics_interpolation_mode='2')
m.node('Camera3D','Camera3D','CameraRig',position='Vector3(-29.698485,42,29.698485)',rotation_degrees='Vector3(-45,-45,0)',projection='1',size='31.0',near='0.1',far='110.0',current='true')
m.node('AudioListener3D','AudioListener3D','CameraRig',position='Vector3(0,6,0)',rotation_degrees='Vector3(0,-45,0)')
...
m.instance('RallyMarker','.','rally',position='Vector3(-10,0,24)',physics_interpolation_mode='2')
...
m.node('ConstructionNavigation','Node','.',script='ExtResource("construction_nav")')
m.instance('BuildingPreview','.','ghost',visible='false')
m.node('Resources','Node3D','.')
for i,(x,z) in enumerate([(-31,17),(-9,27),(-17,2),(9,3),(31,-19)]):m.instance('GoldVein'+str(i),'Resources','mine',position=f'Vector3({x},0,{z})')
m.node('Units','Node3D','.')
starts=[]
...
starts += [('farmer',0,-18,29),('farmer',0,-16,30)]
...
m.node('IncomeTimer','Timer','.',wait_time='1.0',process_callback='0')
m.node('EnemyTimer','Timer','.',wait_time='100.0',process_callback='0')
