...
| musket_shot | 2 | 0.72 s | -18.5 dBFS |
...
活动 RMS 使用 20 ms 音频块，统计能量在最响块 20 dB 范围内的片段；这是便于短音效匹配的测量，并非 LUFS。全部成品活动 RMS 与目标相差不超过 0.01 dB。普通战斗为 −20 dBFS、火枪/炮/爆炸为 −18.5 dBFS、UI/金币/胜负为 −22 dBFS、步伐/马蹄/车轮为 −24 dBFS。战斗 4 倍真峰值不高于 −3 dBTP，UI 和移动音效不高于 −6 dBTP；56 个文件均无削波，首尾采样为零。
...
火枪使用独立的干脆爆响、中低频冲击和短尾音，搭配轻微枪机拟音，不再升调复用炮声。只重建火枪用 `python tools/build_audio.py --only musket_shot`；其他 WAV 和清单记录保持原样。运行增益 −2 dB，110 ms 同类触发间隔、最多 3 个同时播放，沿用 Combat 压缩器和 Master 限幅器。
