...
该轮音效库为 54 个 WAV、21 类素材，运行时增加复用投石样本的轻石击 `stone_chip`，共 22 类事件。81 份原始录音逐字节对照下载包核验，54 个成品离线重建后的 SHA-256 全部一致；许可与来源见 [CC0 音源记录](../assets/audio/CREDITS.md)。当时未接入 BGM；2026-09-25 起积木战争使用独立音乐总线和原生循环播放器，见 [正式曲目记录](audio/block_war_music.md)。
...
2026-09-25 战斗 BGM1：设置 162 项、正式积木战争音乐播放与生命周期 16 项、隔离音乐后的战斗音效混音 58 项、原模式设置集成 60 项全部通过。音乐验证涵盖原生 MP3 曲尾回环、暂停继续、独立静音、零音量、音效仍可输出、退回主菜单释放和再次进入。音效测试等待捕获器初始化，并在镜头瞬移后重置插值、等待物理帧，避免把混音/相机初始化时序当作功能失败。Vulkan 完成 6 张设置截图与真实窗口尺寸回退，声音页已检查无裁切。
