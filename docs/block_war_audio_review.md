...
Q「迅猛冲刺」在半径 2.4 米的落点成功选中自己的露天士兵时，播放一次独立风声与短纸张扑动。空落点拒绝施法，不触发成功施法声；6 秒增益随所选士兵移动，后续进圈者不补领，也不重新播放选兵声。红色速度线与轮廓负责持续反馈，保留阵营标识。
...
| 兔子 Q · 迅猛冲刺 | 成功选兵时播放一次风声与短纸张扑动，使用独立事件；随兵增益期间不反复触发 | [播放](audio/block_war/rabbit_dash.wav) |
...
历史验证结果（早于兔子 Q/E/R 改版）：原生混音 68 项、音效触发 28 项、输入回归 25 项、兔子技能集成 92 项，共 213 项通过。技能测试显式补满测试双方的技力，避免依赖当前 30 点的开局设定；实战初始技力不变。1280×720 私有桌面截图确认帮助页署名未超出面板。
