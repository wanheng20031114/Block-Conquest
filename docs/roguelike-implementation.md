...
验证结果与复现方式见 [肉鸽验证记录](roguelike-validation.md)，真实战斗样本见 [首轮战斗基线](../report/rogue-battle-baseline.md)。

## 已实现的玩法

- 开局先选三种战略之一，再选三套 18 人口的初军之一；初始等级 1、人口上限 20、金币 20、面包 3、行动力 12、招募券 1。初军全部出战，新招募单位进入不限人口的待命区。
- 森林地图使用 arc-nice 的固定 `4c` 拓扑：34 个节点、37 条双向道路，无角色小人。地图节点与金币、吐司、鞋子图标使用 3D 模型；收藏品位于下栏。鼠标选择节点预览，中键平移，滚轮缩放。
...
前哨站是难度 1 的专用横向地图，西侧部署，敌方固定 5 座箭塔、3 座兵营；普通有 16 名守军，紧急有 20 名，并对敌方生命/基础攻击应用 `+25% / +15%`。部分敌军驻守、部分主动搜索。目标是同时清空敌方单位和建筑。进入时有原生相机动画、NPC 目标说明，以及可跳过开场和持续目标面板。难度是独立的 `1–5` 整数字段，与楼层和紧急变体分开保存；首版所有节点和两张战场均为难度 1，紧急作战不改变该值。
...
- `data/rogue/battles/outpost.tres`、`siege.tres`：专用战场、基地与波次；共用默认字段由 `RogueBattleDefinition` 提供。
