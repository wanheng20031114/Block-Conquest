...
- 声音：总音量、总静音；首次启动和恢复默认均为 50%、声音开启。另有独立的背景音乐开关及音乐音量，默认开启、50%；音乐音量为零时完全静音。积木战争默认循环战斗 BGM1，暂停中继续，退出或重开时释放旧播放器。旧配置自动补齐音乐默认值，保留已有总音量和静音偏好。
...
音乐使用 `music_enabled`、`music_volume_percent`，沿用 `snapshot()` / `apply_preferences()` 的草稿、保存及显示回退流程。原生 BGM 总线直接发送至 Master，音乐设置不改变 UI / Combat / Foley；总音量控制所有总线。音乐场景与来源见 [战斗 BGM1](../assets/audio/block_war/music/CREDITS.md)。
...
`tests/block_war_music_test.gd` 使用正式战场验证自动播放、MP3 原生曲尾循环、BGM 总线路由、暂停继续、音乐静音时 UI 仍有输出、零音量、恢复播放位置、回主菜单释放及重新进入时仅有一个播放器；Dummy 驱动仍执行真实混音，不向系统扬声器播放。
