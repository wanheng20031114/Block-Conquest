...
- Esc 暂停/继续；F1 打开/关闭玩法说明。暂停时产兵、行军、升级施工、技能冷却和持续时间均停止。
...
选中己方建筑后，建筑旁的浮动图标显示下一级与驻军消耗，悬停查看说明；人口不足或满级时禁用升级。住宅最高四级，炮塔与铁匠铺仍最高三级、升级仍分别消耗 30、60 名驻军。每高一级额外增加 10% 建筑守备。两个改建图标直接选择另一种建筑，消耗 30 名驻军并重置为一级。升级/改建不会破坏已发出的队伍。

每次升级均需 **10 秒**：开工时扣除一次驻军，施工期间保留原等级的模型、产速、停产值和战斗加成，完工后一起切换。浮动升级图标下方以 `10s` 至 `1s` 显示剩余时间，施工期间不能重复升级或改建；派兵、增援和技能仍可使用。不同建筑独立施工，取消选中不影响进度；暂停或战局结束会冻结施工和粒子。跨越完工时刻的长帧按旧、新等级分别计算产兵，征召效果也遵循相同边界。

已占领的炮塔自动拦截射程内敌方行军部队；一级至三级射程分别为 11、12、13 米，开火间隔为 1.5、1.2、0.9 秒，每发分别消灭 1、2、3 人。没有目标时保持待击，实际开火后才装填。中立炮塔不主动开火，炮塔也不会轰击建筑内的驻军。

民兵默认以 3.1 米/秒行军（原为 3.45，降低约 10%），保持竖直长矛，从建筑朝向路径的一侧逐排出发，展开六列编队。开阔处沿轻缓弧线行进，转角提前圆滑过渡，按弯曲程度收窄队形；到达目标周界时收拢结算。住宅、炮塔和铁匠铺均可从任意方向进出，无需绕到正门。不同阵营的部队在路上直接穿透；只有炮塔能够拦截路上的敌军。
